...
void  loop () {
  ...
  switch(cmd) {
    ...
    case 36: Serial.println(F("BD_PRIMARE_DISP"));
      for (int i = 0; i < 23; i++) data23[i] = pgm_read_word_near(dataRow36 + i); txIr(1, 23); waitTx(); break;
    ...
    // verificare NON FUNZIONA !!!
      ...
      for (int i = 0; i < 21; i++) data21[i] = pgm_read_word_near(dataRow29 + i);
      for (int i = 0; i < 30; i++){
        irsend.sendRaw(data21, 21, FIR);
        delay(10);
      }
    ...
    case 96: Serial.println(F("DECODER_XORO_UP"));
      for (int i = 0; i < 67; i++) data67[i] = pgm_read_word_near(dataRow96 + i); txIr(1, 67); break;
    case 97: Serial.println(F("DECODER_XORO_LEFT"));
      for (int i = 0; i < 67; i++) data67[i] = pgm_read_word_near(dataRow97 + i); txIr(1, 67); break;
    case 98: Serial.println(F("DECODER_XORO_RIGTH"));
      for (int i = 0; i < 67; i++) data67[i] = pgm_read_word_near(dataRow98 + i); txIr(1, 67); break;
    case 99: Serial.println(F("DECODER_XORO_DOWN"));  
      for (int i = 0; i < 67; i++) data67[i] = pgm_read_word_near(dataRow99 + i); txIr(1, 67); break;
    case 100: Serial.println(F("DECODER_XORO_OK"));
      for (int i = 0; i < 67; i++) data67[i] = pgm_read_word_near(dataRow100 + i); txIr(1, 67); break;
    case 101: Serial.println(F("DECODER_XORO_i"));
      for (int i = 0; i < 67; i++) data67[i] = pgm_read_word_near(dataRow101 + i); txIr(1, 67); break;
    case 102: Serial.println(F("DECODER_XORO_INDIETRO"));
      for (int i = 0; i < 67; i++) data67[i] = pgm_read_word_near(dataRow102 + i); txIr(1, 67); break;
    case 103: Serial.println(F("DECODER_XORO_EPG"));  
    for (int i = 0; i < 67; i++) data67[i] = pgm_read_word_near(dataRow103 + i); txIr(1, 67); break;
    case 104: Serial.println(F("DECODER_XORO_TXT"));  
    for (int i = 0; i < 67; i++) data67[i] = pgm_read_word_near(dataRow104 + i); txIr(1, 67); break;
    case 105: Serial.println(F("DECODER_XORO_VOL+")); 
    for (int i = 0; i < 67; i++) data67[i] = pgm_read_word_near(dataRow105 + i); txIr(1, 67); break;
    case 106: Serial.println(F("DECODER_XORO_VOL-"));
      for (int i = 0; i < 67; i++) data67[i] = pgm_read_word_near(dataRow106 + i); txIr(1, 67); break;
    case 107: Serial.println(F("DECODER_XORO_PAGE+"));
      for (int i = 0; i < 67; i++) data67[i] = pgm_read_word_near(dataRow107 + i); txIr(1, 67); break;
    case 108: Serial.println(F("DECODER_XORO_PAGE-"));
      for (int i = 0; i < 67; i++) data67[i] = pgm_read_word_near(dataRow108 + i); txIr(1, 67); break;
    case 82: Serial.println(F("DECODER_XORO_FR"));
      for (int i = 0; i < 67; i++) data67[i] = pgm_read_word_near(dataRow82 + i); txIr(1, 67); break;
    case 83: Serial.println(F("DECODER_XORO_FF"));
      for (int i = 0; i < 67; i++) data67[i] = pgm_read_word_near(dataRow83 + i); txIr(1, 67); break;      
    case 86: Serial.println(F("DECODER_XORO_PLAY"));
      for (int i = 0; i < 67; i++) data67[i] = pgm_read_word_near(dataRow86 + i); txIr(1, 67); break;
    case 87: Serial.println(F("DECODER_XORO_PAUSE"));
      for (int i = 0; i < 67; i++) data67[i] = pgm_read_word_near(dataRow87 + i); txIr(1, 67); break;   
    case 88: Serial.println(F("DECODER_XORO_STOP"));
      for (int i = 0; i < 67; i++) data67[i] = pgm_read_word_near(dataRow88 + i); txIr(1, 67); break;
    case 90: Serial.println(F("DECODER_XORO_ROSSO"));
      for (int i = 0; i < 67; i++) data67[i] = pgm_read_word_near(dataRow90 + i); txIr(1, 67); waitTx();
